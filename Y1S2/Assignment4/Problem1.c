...
int isBST(BTNode *root, int left, int right)
{
    if (root == NULL)
    {
        return 1;
    }
    if (!(root -> item < right && root -> item > left))
    {
        return 0;
    }
    return (isBST(root -> left,left, root -> item) && isBST(root -> right,root ->item, right));
}


int bstSize(BTNode *root)
{
    int count = 1;
    if (root == NULL)
    {
    ...
    {
        count += bstSize(root -> left);
        count += bstSize(root -> right);
        return count;
    }
}

BTNode *max(BTNode *root1, BTNode *root2)
{
    if (bstSize(root1) >= bstSize(root2))
    {
        return root1;
    }
    else
